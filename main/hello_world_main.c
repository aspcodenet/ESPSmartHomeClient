...
#define SEND_BUTTON_PIN_RIGHT 26
    ...
    int lastValue = 0;
     ...
     while(1){
        ...
        if(lastValue == 1 && currentValue == 0){
            ...
            if(currentValue == 0){
                //Get value
                ESP_LOGE("SEND","Press LEFT");
                int id = esp_mqtt_client_publish(client, TOPIC, "LEFT"  , 0, 1, 0);
...
void sendTaskRight(){
    gpio_reset_pin(SEND_BUTTON_PIN_RIGHT);
    gpio_set_pull_mode(SEND_BUTTON_PIN_RIGHT, GPIO_PULLUP_ONLY);
    int currentValue = gpio_get_level(SEND_BUTTON_PIN_RIGHT);
    int lastValue = 0;

     while(1){
        lastValue = currentValue;
        //Check button
        currentValue = gpio_get_level(SEND_BUTTON_PIN_RIGHT);
        //ESP_LOGE("main","Press %d", currentValue);
        if(lastValue == 1 && currentValue == 0){
            vTaskDelay(20 / portTICK_PERIOD_MS);
            currentValue = gpio_get_level(SEND_BUTTON_PIN_RIGHT);
            if(currentValue == 0){
                //Get value
                ESP_LOGE("SEND","Press RIGHT");
                int id = esp_mqtt_client_publish(client, TOPIC, "RIGHT"  , 0, 1, 0);
                ESP_LOGE("SEND","ID %d",  id);
            }
        }
     }
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore (
        sendTaskRight,     // Function to implement the task
        "sendTaskRight",   // Name of the task
        4096,      // Stack size in words
        NULL,      // Task input parameter
        0,         // Priority of the task
        NULL,      // Task handle.
